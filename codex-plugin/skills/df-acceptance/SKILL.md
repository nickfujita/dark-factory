...
Create the evidence directory the skill's Evidence section names. When it names none, fall back to `.dark-factory/qa-screenshots/<feature-slug>/`.
...
**Failure capture.** Write failure artifacts where the Evidence section says, or to `.dark-factory/qa-screenshots/<feature-slug>/` when it says nothing. Capture on failure only.
