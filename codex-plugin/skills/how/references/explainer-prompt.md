...
Build the explainer subagent's prompt from this template. Fill in the placeholders. For a simple question answered in-session (Step 2b), apply the same instructions, output format, and communication style yourself, with your own exploration in place of explorer findings.
...
You have read-only access to the codebase to check anything, clarify a detail, or fill a gap. Use Read, Grep, and Glob as needed. Do not write or modify anything. The explorers did the heavy lifting, so you shouldn't need to re-explore from scratch.
