...
4. **Scale.** Spawn a rolling window of workers up to the 3-in-flight cap, refilling as children finish; blocking batches pay the slowest child of every batch. Recompute ready work after each drain, relay upstream reports into downstream briefs, and keep sibling communication upward only. Before dispatching a dependent unit, enforce the predecessor-readiness gate in `playbooks/df-open-pr.md`; a worker completion alone does not release its dependents.
5. **Drain.** Run the queue discipline below at every drain point.
6. **Land.** Finish each PR's local work, then mark it ready for human review and own its CI watcher through `playbooks/babysit.md`. Batch agent-ready PRs into the status report. The dependency frontier is the lowest PR whose agent-owned readiness gates are incomplete. Apply the predecessor-readiness gate in `playbooks/df-open-pr.md` before dependent work. Human review and merge are tracked separately and need not block the next layer once that gate passes. Only the operator merges.
...
- Drain in batches at four points: the end of a critical section, a wave boundary, a bounded-polling-loop wake, and before an operator report. Arrivals during a drain wait for the next one.
