...
1. Capture the live signal on the matching surface: a CPU profile for a spinning process, a heap snapshot for a leak, a devtools trace for a visual glitch. Drive the surface through the project verification skill when `.dark-factory/project.yaml` names one, otherwise the closest surface you can reach yourself, such as a local run or agent-browser for UI. A real artifact, not a guess.
