...
|----------|-----------|-------|
| Codex | native Codex subagent, background spawn | the `discovery_reviewers` responsibility in `../df/references/model-policy.md` |
| Claude | claude tmux adapter (the cross-model review leg) | the codex section of `../df/references/model-policy.md` |
...
The Claude leg runs through the D26 transport contract: brief and artifact digest in, structured findings and a terminal status out, an interactive tmux session (never `claude -p`), a review-only instruction, a completion sentinel with structural validation, and a deadline with a reaper around it.
