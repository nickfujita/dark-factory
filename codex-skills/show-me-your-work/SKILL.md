...
At the end of the run, before handing back, check the log told the truth. The record to audit against is the run itself: the tool calls and results in this session's history, the artifacts on disk, and the git history the run produced. Walk the log against what actually happened:

- Every row maps to a real action. Cut invented or aspirational entries.
- Each row's evidence resolves and shows what the row claims. Re-open the pointer on disk; don't audit from memory.
...
Before handing back, the trail gets fresh eyes from the other model family. Self-review is not a substitute; the point is a reviewer you cannot bring yourself. Send the review through the cross-model transport named in the df model policy (`../df/references/model-policy.md`). The reviewer gets the decision log, the evidence pointers, and a short run summary, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.

- Decisions logged with weak or absent evidence.
- Verification steps claimed without proof behind the evidence pointer.
- Choices that look risky in hindsight (premature, scope-creeping, papering over a symptom).
- Gaps the user would otherwise miss on a casual skim.

Every reply for a run that produced a trail ends with an "Attention" section. Lead with the reviewer's model on its own line (`reviewed by <model>`), then list each flag pointing to specific rows or moments. "No flags" is a valid value; the model name is not. When the transport is unavailable or its usage window is exhausted, the line reads `review deferred: <reason>` instead, and the deferral is itself a flag for the user. Do not substitute a same-family reviewer; a correlated draw is not fresh eyes. The self-audit asks if the log told the truth; this asks what the user should still scrutinize even when it did.
