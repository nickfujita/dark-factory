...
You may read the codebase and call MCP tools to verify citations. Do not write files, commit, or modify external state.
...
5. **Verify citations by spot-checking.** If you're uncertain a cited item exists or says what's claimed, check it. Don't propagate errors.
...
**Explicit gaps.** Things the user asked that the evidence didn't answer. Sources searched that came up empty. Sources that weren't searchable at all, such as a category with no matching MCP.
