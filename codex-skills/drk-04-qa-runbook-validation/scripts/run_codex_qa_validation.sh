...
# PRD and QA runbook — no content inlined. Fail closed if read-only sandboxing
# is unavailable; review helpers must not silently become write-capable.
...
if ! grep -q '^## Findings — Codex CLI' "$out_path"; then
