...
1. Read `references/prd-template.md` for the exact output format.
   (This file is in the skill directory: `${CODEX_SKILLS_HOME:-${CODEX_HOME:-$HOME/.codex}/skills}/drk-01-prd-interview/references/`
   or the repo's `codex-skills/drk-01-prd-interview/references/` directory.
...
- If all pass: set Status to "Hardened", complete Phase 5 output, then
  proceed to Phase 6 to hand the artifact path back to the Codex orchestrator.
...
and status. When running under `dark-factory-codex`, stop here so the
orchestrator can explicitly invoke `drk-02-prd-challenge` with that path. If
