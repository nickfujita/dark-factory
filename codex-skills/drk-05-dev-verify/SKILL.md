...
description: "Developer self-verification before code review: runs all tests and QA runbook inline, logs failures, fixes them in a loop, then returns code-review-ready status. Use after implementation is complete and before submitting for multi-context code review."
---
...
- Ask: mark the branch ready for `drk-06-code-review` anyway, or stop to investigate?
...
`dark-factory-codex`, stop here so the orchestrator can explicitly invoke
`drk-06-code-review`. If this skill was invoked standalone, tell the user the
next stage is `drk-06-code-review`.

## Notes

- The issues doc at `.dark-factory/tmp/dev-verify-issues.md` is a working scratch
  file — it is not committed
- Run QA inline (not by invoking drk-07-qa-acceptance) to keep the fix loop
...
- This skill verifies the developer's own work before review, not after. The
  multi-context code review in drk-06-code-review is a second opinion on
