...
for s in df-state.sh df-check-leakage.sh df-stack.sh df-role.mjs check-model-policy.mjs test-df-role-policy.sh df-selection.mjs test-df-selection.sh; do
...
for ref in run-state-schema.md engineering-standards.md model-policy.json verification-selection.schema.json; do
