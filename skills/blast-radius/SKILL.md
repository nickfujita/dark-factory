...
5. Prove the one fact. Write a script or test that runs the real code, run it, and paste what happened. Proof scripts live in the scratchpad or a throwaway path and change nothing in the project. If you can't prove it cheaply, mark it unproven. Don't round up.
6. For a big or wide change, get a second opinion from the other model family through the cross-model transport in the df model policy (`../df/references/model-policy.md`), same question, and merge the answers. Different families catch different real bugs. The `arena` skill owns the multi-candidate version and is pending port; say so if a case genuinely needs more than the two families.
