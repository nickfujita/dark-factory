...
6. For a big or wide change, get a second opinion from the other model family through the cross-model transport in the df model policy (`../df/references/model-policy.md`), same question, and merge the answers. Different families catch different real bugs. The `arena` skill owns the multi-candidate version, High-consequence and operator-invoked.
