...
Spawn all explorers with the Agent tool in a single message. For each:
...
Explore and explain in-session. Use Glob, Grep, and Read yourself and write the explanation directly. A simple question does not earn a spawn; the session does small tasks itself. Read `references/explainer-prompt.md` for the communication style and output format. Same structure, just no explorer findings as input.

Proceed to Step 4.

### Step 3. Synthesize (complex questions only)

Once all explorers return, spawn a single explainer with the Agent tool to synthesize their findings into one coherent explanation:
