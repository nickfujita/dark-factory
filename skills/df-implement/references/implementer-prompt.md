...
The controller builds each implementer dispatch from this template. Spawn with the Agent tool in the background. Preflight the frozen `implementation_delegate` role through `../../df/references/model-policy.md`; a transcription task, where the plan text contains the complete code, takes the menial tier instead. A role marked inherit omits the model field. The native harness reserves the dispatch before spawning.
