...
The controller builds each implementer dispatch from this template. Spawn with the Agent tool in the background. Resolve the model through the `implementation_delegate` role in `../../df/references/model-policy.md`; a transcription task, where the plan text contains the complete code, takes the menial tier instead. A role marked inherit omits the model field. Reserve the dispatch through `scripts/df-state.sh` before spawning.
