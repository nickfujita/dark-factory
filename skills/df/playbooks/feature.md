...
8. **Per-PR delivery loop.** Repeat steps 8 through 11 for each eligible PR, not once for the entire feature. Apply the predecessor-readiness gate in `playbooks/df-open-pr.md` before dependent implementation. Independent PRs may proceed in parallel. **Implement.** Run `df-implement`, which lands in a parallel wave of this port. It owns delegation, the per-task review economy, and the TDD discipline. Review every delegate's diff yourself.
...
11. **Open PRs.** Run `playbooks/df-open-pr.md` per PR. Small PRs merge into main behind the feature flag as each goes green, typically three to seven. The first PRs deliver a visible vertical slice. Independent PRs branch from main and stay outside a stack. Register only a genuine dependent branch chain as a native GitHub stack, when the capability probe succeeds. Each PR gets its own review and live verification of its changed surface. A new head SHA from GitHub's cascading rebase repeats the applicable gates. Verified-but-unlanded work counts as zero, so PRs merge as they land rather than waiting for the whole chain.
