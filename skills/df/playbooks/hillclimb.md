...
5. Loop, one hypothesis per iteration.
   ...
   Each iteration ends in a check before the next begins, per the sequence-verifiable-units principle. If the run is unattended, borrow only the wake mechanism from `playbooks/autonomous-run.md`, a Monitor until-loop, not its stop rule. This playbook's stop criteria govern, so a plateau means pivot, not stop.
