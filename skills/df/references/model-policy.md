...
A pinned role never runs above the current session model unless it is a designated floor. Floors are the only pins allowed to exceed a throttled session, because their job is to keep a review meaningful. `drk-reviewer-recheck` is the only floor today, marked `# floor` below.
...
claude:
  ...
  recheck_leaf_reviewers: drk-reviewer-recheck  # agent definition; floor
  eval_graders: sonnet
